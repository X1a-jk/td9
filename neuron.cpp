...
void Neuron::step()
{
    ...
    // double sum =-dendrites[0]->getSignal();
    double sum = 0.0;
    int n = dendrites.size();
    for (int i = 0; i < n; i++)
    {
        sum += dendrites[i]->getSignal() * weights[i];
    }
    collected_input = sum;
    double val = activation(collected_input);
    axon->setSignal(val);
}
...
void Neuron::step_back()
{
    ...
    double back = axon->getBackValue() * activation_der(collected_input);
    int n = dendrites.size();
    for (int i = 0; i < n; i++)
    {
        dendrites[i]->setBackValue(back * weights[i]);
        weights[i] -= rate * back * dendrites[i]->getSignal();
    }
}
