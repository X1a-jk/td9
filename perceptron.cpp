      ...
      rate(_rate), decay(_decay)
{
    ...
    {
        Neuron *neu = new Neuron(dim, inputs, _activation, _activation_der);
        hidden[i] = neu;
    }
...
void OneLayerPerceptron::prepareInputs(Dataset *data, int row, int regr, bool print)
{
    ...
    std::vector<double> data_row = data->getInstance(row);
    ...
    for (int i = 0, j = 0; i < dim + 1; i++, j++)
    {
        ...
        temp->setSignal(normalise(data_row[i], data, i));
    }
    // TODO Exercise 4
